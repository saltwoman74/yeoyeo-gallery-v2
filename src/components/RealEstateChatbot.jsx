...
const RealEstateChatbot = () => {
    const [viewState, setViewState] = useState('initial'); // initial, categories, question, answer
...
보금자리론은 2026년 1월부터 0.25%p 인상되어 비우대 기준 약 3.9~4.2%, 우대금리 적용 시 2.9~3.2% 수준이며, 실수요자는 정책모기지(보금자리/디딤돌 등) 우선 검토 후, 모자라는 부분을 시중은행 혼합형으로 메우는 구조가 유리하다.`,
                ...
                },
                {
                    ...
                    a: `창원시는 현재 대부분 비규제지만, 일부 지역은 조정대상지역 또는 규제 논의 이력이 있으며, 입주 감소+매매가 급등이 겹치면 조정대상지역·투기과열지구 지정 가능성이 생긴다.
...
중개사가 입장에서는 고객에게 “남들 다 오를 때 따라 사는 게 아니라, 데이터 상으로 유리할 때, 미리 준비된 사람만 매수하는 것이 진짜 현명한 행동”이라는 메시지를 꾸준히 교육해 두면, 나중에 실제 거래 상담이 훨씬 수월해진다.`,
    ...
    const handleOpenImageApp = () => {
        window.open('https://unicity-iota.vercel.app', '_blank');
    };
    ...
    const handleQuestionSelect = (question) => {
        ...
        // Add to specific questions history if needed, for now just simple view
    };

    const handleFollowUp = (questionId) => {
        // Find question across all categories
        for (const [key, cat] of Object.entries(knowledgeBase)) {
                ...
                {viewState === 'question' && currentCategory && (
                    <div className="animate-slide-in-right">
                        ...
                        <div className="space-y-3">
                            {knowledgeBase[currentCategory].questions.map((q) => (
                                <button
                                    key={q.id}
                                ...
                                <div className="flex flex-col gap-2">
                                    {currentQuestion.followUp.map(id => {
                                        ...
                                        for (const cat of Object.values(knowledgeBase)) {
                                            const found = cat.questions.find(fq => fq.id === id);
                                            if (found) {
                                                suggestion = found;
                                                break;
                                            }
